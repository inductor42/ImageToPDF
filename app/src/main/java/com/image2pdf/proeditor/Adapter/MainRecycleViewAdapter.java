...
public class MainRecycleViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private void toggleCheckedIcon(RecyclerView.ViewHolder holder, int position) {
        ...
        if (selected_items.get(position, false)) {
            view.lyt_parent.setBackgroundResource(R.drawable.background2);
            if (current_selected_idx == position) resetCurrentIndex();
        } else {
            view.lyt_parent.setBackgroundResource(R.drawable.background3);
            if (current_selected_idx == position) resetCurrentIndex();
        }
    }
    ...
    public String GetSize(long size) {
        String[] dictionary = {"bytes", "KB", "MB", "GB", "TB"};
        int index = 0;
        double m = size;
